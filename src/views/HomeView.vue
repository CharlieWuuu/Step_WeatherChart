<template>
  ...
  <div class="weatherChart_group_container">
    <div class="weatherChart_container" v-for="(l, index) in location">
      <div class="title_container">
        <p>
          {{ l.station.StationName }}
          ｜
          {{
            l.station.StationName == "玉山"
              ? "高地氣候"
              : chartData_Arr[index].datasets[1].data[0] < 18
              ? "副熱帶季風"
              : "熱帶季風"
          }}
        </p>
    ...
    async getClimateData() {
      ...
      for (var i = 0; i < location.length; i++) {
        ...
        const Precipitation_Arr =
          stationObsStatistics.Precipitation.monthly.map((element) =>
            parseFloat(element.Accumulation)
          );
        ...
        console.log(this.chartData_Arr[i].datasets[1].data[0]);
      }
